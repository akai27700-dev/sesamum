...
from othello_gui_dialogs import StartupSettingsDialog
from othello_gui_search import OthelloSearchMixin

BASE_DIR = core.BASE_DIR
BEST_MODEL_PATH = core.BEST_MODEL_PATH
BLEND_CALIBRATION_LOG_PATH = os.path.join(BASE_DIR, "blend_calibration_samples.jsonl")
DEVICE = core.DEVICE
...
class UltimateOthello(OthelloSearchMixin):
    def __init__(self, r):
        ...
        self.super_light_mode = settings.get("super_light_mode", False)
        self.use_tt_resume = settings["use_tt"]
        self.time_limit_sec = settings.get("time_limit", 10.0) if not settings.get("auto_time") else 10.0
        self.auto_time = settings.get("auto_time", False)
        self.auto_mode_type = settings.get("auto_mode_type", "normal")
        ...
        self.opening_book = {}
        book_path = os.path.join(BASE_DIR, "opening_book.json")
        self.log(f"定石データを読み込み中: {book_path}")
        ...
        time_str = f"{self.time_limit_sec:.1f}s" if not self.auto_time else "auto"
        mcts_influence_str = "auto%" if self.auto_time else f"{self.mcts_influence_pct}%"
        self.log(f"探索設定: TT size={TT_SIZE * 2}, C++={'ON' if self.use_cpp_engine else 'OFF'}, NN={'ON' if self.use_nn else 'OFF'}, MCTS={'ON' if self.use_mcts_enabled else 'OFF'}, MCTS influence={mcts_influence_str}, book={int(self.opening_book_rate*100)}%, light mode={'ON' if self.light_mode else 'OFF'}, super light mode={'ON' if self.super_light_mode else 'OFF'}, TT reuse={'ON' if self.use_tt_resume else 'OFF'}, time limit={time_str}, exact solve starts=always-auto, pondering={'ON' if self.use_pondering else 'OFF'}")
    ...
    def get_auto_time_limit(self, empty, legal_count, mvs):
        ...
        # Super Lightモードの制限時間を0.6秒に設定
        if self.super_light_mode or self.auto_mode_type == "super_light":
            base_time = 0.6
            max_time = 0.6
        # 完全自動モード判定
        # 空きマス数と手数に基づいて自動的にモードを決定
        elif empty >= 48:  # 序盤
            base_time = 3.0
            max_time = 6.0
        elif empty >= 36:  # 中盤前半
        ...
        # 合法手数による調整
        if legal_count <= 4:
            base_time *= 0.7  # 合法手が少ない場合は短縮
        elif legal_count >= 12:
            base_time *= 1.3  # 合法手が多い場合は延長
            
        empty_factor = max(0.7, min(1.3, empty / 32.0))
        move_factor = max(0.85, min(1.2, legal_count / 10.0))
        game_phase = min(60, mvs + 4) / 60.0
        phase_factor = 0.8 + 0.4 * (1.0 - game_phase)
        auto_limit = base_time * empty_factor * move_factor * phase_factor
        return max(1.0, min(max_time, auto_limit))
    ...
    def should_use_opening_book(self, mvs, empty):
        """自動定石使用判定"""
        # 序盤のみ定石を使用
        if mvs <= 8:
            return 0.9  # 90%の確率で使用
        elif mvs <= 12:
            return 0.7  # 70%の確率で使用
        elif mvs <= 16:
            return 0.5  # 50%の確率で使用
        elif mvs <= 20:
            return 0.3  # 30%の確率で使用
        else:
            return 0.0  # 定石を使用しない
    ...
    def should_start_exact_early(self, aB, oB, empty, legal_count):
        # 自動読み切り開始条件
        if empty <= 10:
            return True  # 終盤は必ず読み切り（2手遅らせる）
        
        # Normalモードの基準：残り22手で合法手4手以下、10手で合法手22以下で開始
        if empty <= 22:
            threshold = int(22 + (empty - 10) * (-2.25))  # empty=22→4, empty=10→22
    ...
    def drw(self, probs_map=None):
        ...
        elif self.tn == self.ac and self.sp_var.get() and probs_map:
    ...
    def ai_r(self, current_game_id):
        try:
                ...
                for swap in (False, True):
                    ...
                    for op in range(8):
                        ...
                        if state_key in self.opening_book:
                            book_roll_used = True
                            if self.light_mode or self.super_light_mode or np.random.random() < self.should_use_opening_book(mvs, empty):
                                book_entry = self.opening_book[state_key]
                                if isinstance(book_entry, dict) and "moves" in book_entry:
                                    moves_list = book_entry["moves"]
                                    probs_list = book_entry.get("probs", [1.0/len(moves_list)]*len(moves_list))
                                    if len(moves_list) >= 3:
                                        top_moves = moves_list[:3]
                                        top_probs = probs_list[:3]
                                        total_prob = sum(top_probs)
                                        top_probs = [p/total_prob for p in top_probs]
                                        selected_idx = np.random.choice(len(top_moves), p=top_probs)
                                        bm_rot = top_moves[selected_idx]
                                    else:
                                        bm_rot = moves_list[0]
                                else:
                                    bm_rot = book_entry if isinstance(book_entry, int) else book_entry[0] if isinstance(book_entry, list) else list(book_entry.values())[0] if isinstance(book_entry, dict) else 0
                                book_move = unrotate_move(bm_rot, op)
                            break
            ...
            auto_time_str = " (auto)" if self.auto_time else ""
            ...
            self.log(self.format_log_columns([
                ...
                f"limit={time_limit:.1f}s{auto_time_str}",
            ...
            if book_move is not None:
                ...
                book_rate = self.should_use_opening_book(mvs, empty)
                self.log(f"book: hit but skipped by {int(book_rate*100)}% rule")
            if is_resumed:
            ...
            if best_move != -1:
                if book_move is not None or (mvs == 0 and best_move in rms):
                    self.log(f"selected by book/standard: {self.format_move_label(best_move)}")
