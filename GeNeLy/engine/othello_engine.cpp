...
inline bool try_nn_pruning(Bitboard p, Bitboard o, int mvs, int depth, double alpha, double beta, bool is_exact, SearchContext& ctx) {
    ...
    // NN評価に基づく枝刈り閾値
    double nn_margin = 50.0 * (depth - 2); // 深さに応じて閾値を調整
    double confidence = std::min(0.3, 0.1 * depth); // NNの信頼度
    
    // Alphaカット: NN評価がalphaより十分高い場合
...
// NNベースのMove Ordering - NN評価値を使って手の順序付け
...
std::pair<double, std::int64_t> alphabeta(Bitboard p, Bitboard o, int mvs, int depth, double alpha, double beta, bool passed, bool is_exact, SearchContext& ctx) {
    ...
    // NNベースのMove Orderingを使用
    std::vector<std::pair<int, double>> nn_ordered_moves;
    if (ctx.nn_enabled && depth >= 2) {
        ...
        while (v_temp) {
            ...
            // Enhanced move ordering - corner and edge bonuses
...
std::pair<std::vector<double>, std::vector<std::int64_t>> search_root_parallel_impl(Bitboard p, Bitboard o, int mvs, int depth, bool is_exact, const std::vector<int>& ordered_indices, SearchContext& ctx) {
    ...
    const int lane_count = estimate_root_parallel_lanes(root_order.size(), depth, is_exact);
