...
class OthelloSearchMixin:
    ...
    def should_stop_pondering(self, current_game_id, ponder_token, ponder_sf, force_stop=False):
        ...
        return not self.running or self.game_id != current_game_id or self.ponder_token != ponder_token or (self.tn == self.hc) or stop_flag
    ...
    def merge_ponder_cache_entry(self, cache_key, ordered_moves=None, completed_depth=None, mcts_res=None, best_mcts_wr=None, root_visits=None, mcts_sim_count=None):
        with self.ponder_lock:
            ...
            if completed_depth is not None:
                safe_depth = self.clamp_search_depth(completed_depth)
                entry['completed_depth'] = max(self.clamp_search_depth(entry.get('completed_depth', 2)), safe_depth)
            if mcts_res:
    ...
    def stop_pondering_explicitly(self):
        ...
        with self.ponder_lock:
            if self.ponder_sf is not None and isinstance(self.ponder_sf, np.ndarray):
                self.ponder_sf[0] = 1
            mcts_thread = self.ponder_mcts_thread
            ab_thread = self.ponder_ab_thread
        self.ponder_token += 1
        if mcts_thread and mcts_thread.is_alive():
            mcts_thread.join(timeout=0.1)
        if ab_thread and ab_thread.is_alive():
            ab_thread.join(timeout=0.1)
    ...
    def finish_ponder_worker(self, worker_name, ponder_token):
        ...
        with self.ponder_lock:
            ...
            if self.ponder_mcts_thread is None and self.ponder_ab_thread is None and (self.ponder_active_token == ponder_token):
                self.ponder_active_token = -1
                should_restart = self.running and getattr(self, 'use_pondering', True) and (self.tn == self.hc) and (self.ponder_token == ponder_token)
        if should_restart:
            try:
                self.call_on_ui_thread(lambda: self.rt.after(25, self.start_pondering))
            except Exception:
                pass
    ...
    def ponder_mcts_r(self, current_game_id, ponder_token, ponder_sf, candidates):
        try:
            ...
            if DEVICE_STR == 'cuda':
                cycle_budget = max(2.8, self.time_limit_sec * (0.8 if self.light_mode else 1.25))
                cycle_budget = min(12.0, cycle_budget)
                warm_slice = 0.45 if self.light_mode else min(1.1, max(0.55, self.time_limit_sec * 0.08))
            else:
            ...
            while not self.should_stop_pondering(current_game_id, ponder_token, ponder_sf) and round_idx < max_rounds:
                for rank, candidate in enumerate(candidates):
                    ...
                    try:
                        ...
                        cached_mcts_res, cached_best_mcts_wr, cached_mcts_sim_count, _, _, cached_root_visits = get_mcts_win_rates_time_batched(self.nn_model, candidate['αβ'], candidate['oB'], self.ac, candidate_time, ponder_batch_size, ponder_sf)
                    except Exception:
                        cached_mcts_res = {}
    ...
    def ponder_ab_r(self, current_game_id, ponder_token, ponder_sf, candidates):
        try:
            ...
            if self.light_mode:
                cycle_budget = max(1.1, self.time_limit_sec * 0.65)
                cycle_budget = min(3.2, cycle_budget)
                warm_slice = 0.16
            else:
            ...
            while not self.should_stop_pondering(current_game_id, ponder_token, ponder_sf) and round_idx < max_rounds:
                for rank, candidate in enumerate(candidates):
                    ...
                    for dp in range(depth_start, depth_cap + 1):
                        ...
                        if remain_ms <= 1:
                            break
                        depth_exact = bool(candidate['is_exact'] and dp >= empty_count)
                        timed_out = False
                        try:
                            if any(candidate['root_policy_vector']):
                                status = cpp_engine.search_root_parallel_cached_status_policy(int(candidate['αβ']), int(candidate['oB']), int(candidate['mvs']), int(dp), bool(depth_exact), curr_ordered, candidate['root_policy_vector'], remain_ms)
                            else:
                                status = cpp_engine.search_root_parallel_cached_status(int(candidate['αβ']), int(candidate['oB']), int(candidate['mvs']), int(dp), bool(depth_exact), curr_ordered, remain_ms)
                            vals = status['vals']
    ...
    def _run_cpp_search_session(self, aB, oB, mvs, start_dp, is_exact, rms, root_policy_vector, use_ab, use_mcts_enabled, time_limit, search_profile, current_game_id, is_resumed):
        ...
        def unpack_result(result, ab_enabled, mcts_enabled):
            ...
            for move, value, win_rate in zip(ab_out.get('moves', []), ab_out.get('values', []), ab_out.get('win_rates', [])):
                ab_res_local[int(move)] = float(win_rate)
                ab_val_res_local[int(move)] = float(value)
            completed_depth_local = int(ab_out.get('completed_depth', max(2, start_dp - 1)))
            attempted_depth_local = int(ab_out.get('attempted_depth', completed_depth_local))
            if ab_enabled:
                self._last_ab_val_res = ab_val_res_local
                self._last_ab_completed_depth = completed_depth_local
                if attempted_depth_local > completed_depth_local:
                    self.log(self.format_log_columns(['ab: done', f'completed={completed_depth_local}', f'attempted={attempted_depth_local}'], [8, 15, 15]))
                else:
                    self.log(self.format_log_columns(['ab: done', f'completed={completed_depth_local}'], [8, 15]))
            else:
                self._last_ab_val_res = {}
                self._last_ab_completed_depth = 2

            mcts_res_local = {int(k): float(v) for k, v in dict(mcts_out.get('move_win_rates', {})).items()}
            root_visits_local = {int(k): int(v) for k, v in dict(mcts_out.get('root_visits', {})).items()}
            best_mcts_wr_local = float(mcts_out.get('best_wr', 50.0))
            mcts_sim_count_local = int(mcts_out.get('simulation_count', 0))
            if mcts_enabled:
                nn_batch_count = int(mcts_out.get('nn_batch_count', 0))
                top_visits = sorted(root_visits_local.items(), key=lambda x: x[1], reverse=True)[:5]
                top_visit_str = ' | '.join([f'{self.format_move_label(m)}={v}' for m, v in top_visits])
                self.log(self.format_log_columns(['mcts: done', f'sims={mcts_sim_count_local}', f'nn_batches={nn_batch_count}', f'best={best_mcts_wr_local:.1f}%'], [10, 14, 16, 11]))
                if top_visit_str:
                    self.log(f'mcts: root visits | {top_visit_str}')
            return {
                'ab_out': ab_out,
                'ab_res': ab_res_local,
                'ab_val_res': ab_val_res_local,
                'mcts_res': mcts_res_local,
                'root_visits': root_visits_local,
                'best_mcts_wr': best_mcts_wr_local,
                'mcts_sim_count': mcts_sim_count_local,
            }

        pruning_mode = bool(use_mcts_enabled and use_ab and not is_exact and self.mcts_pruning_enabled)
        ...
        if use_mcts_enabled:
            ...
            self.call_on_ui_thread(self.rt.title, f'Sesamum - {ai_color}')
        else:
        ...
        if is_exact:
            ab_budget_ms = int(max(50.0, float(time_limit) * 1000.0))
        elif search_profile['ab_budget'] is None:
            ab_budget_ms = 0
        else:
        ...
        if pruning_mode:
            ...
            mcts_result = mcts_session.run(
                ...
                [int(x) for x in rms], [float(x) for x in root_policy_vector],
                False, True, float(mcts_time_limit), int(self._get_live_mcts_batch_size(mcts_time_limit)),
                0.0, 0, max_depth, False, self.sf_arr, infer_leaves, None
            )
            parsed_mcts = unpack_result(mcts_result, False, True)
            mcts_res = parsed_mcts['mcts_res']
            root_visits = parsed_mcts['root_visits']
            best_mcts_wr = parsed_mcts['best_mcts_wr']
            mcts_sim_count = parsed_mcts['mcts_sim_count']
            ranked_moves = sorted(
                [int(move) for move in rms],
                key=lambda move: (root_visits.get(move, 0), mcts_res.get(move, 50.0)),
                reverse=True,
            )
            pruned_rms = ranked_moves[:max(1, int(self.mcts_pruning_branches))] if ranked_moves else [int(x) for x in rms]
            if mcts_res:
                mcts_res = {move: mcts_res[move] for move in pruned_rms if move in mcts_res}
            ...
            ab_result = ab_session.run(
                int(aB), int(oB), int(self.tn), int(mvs), int(max(start_dp, 2)), bool(is_exact),
                [int(x) for x in pruned_rms], [float(x) for x in root_policy_vector],
                True, False, float(ab_time_limit), int(self._get_live_mcts_batch_size(ab_time_limit)),
                0.0, int(max(50.0, ab_time_limit * 1000.0)), max_depth, False, self.sf_arr, infer_leaves, ab_progress
            )
            parsed_ab = unpack_result(ab_result, True, False)
            return {
                'mcts_res': mcts_res,
                'best_mcts_wr': best_mcts_wr,
                'root_visits': root_visits,
                'mcts_sim_count': mcts_sim_count,
                'ab_res': parsed_ab['ab_res'],
                'ab_val_res': parsed_ab['ab_val_res'],
                'resolved_flag': bool(parsed_ab['ab_out'].get('resolved', False)),
            }
        ...
        return {
            ...
            'best_mcts_wr': parsed['best_mcts_wr'],
            'root_visits': parsed['root_visits'],
            'mcts_sim_count': parsed['mcts_sim_count'],
            'ab_res': parsed['ab_res'],
            'ab_val_res': parsed['ab_val_res'],
            'resolved_flag': bool(parsed['ab_out'].get('resolved', False)),
        }
