...
class StartupSettingsDialog:
    def __init__(self, parent, cpp_available):
        ...
        self.use_pondering = tk.StringVar(value="on")
        ...
        self._add_radio_group(frame, "定石", self.book_source, [("Egaroucid", "egaroucid"), ("WTH", "json")])
        self.book_usage_scale = self._add_scale_group(frame, "基本定石使用率(%)", self.book_usage, 0, 100)
    ...
    def _toggle_book_options(self, *_):
        use_egaroucid = self.book_source.get() == "egaroucid"
        self.book_usage_scale.config(state="disabled" if use_egaroucid else "normal")

    def on_ok(self):
        nn_enabled = self.use_nn.get() == "on"
        mcts_pct = self.mcts_influence.get()
        use_egaroucid = self.book_source.get() == "egaroucid"
        book_usage_pct = 100 if use_egaroucid else self.book_usage.get()
        ...
        self.result = {
            ...
            "use_pondering": self.use_pondering.get() == "on",
        }
        self.top.destroy()
