...
def get_legal_moves(P, O):
    m, e, l = np.uint64(0x7E7E7E7E7E7E7E), ~(P | O) & _FULL_MASK, np.uint64(0)
    t = (P << np.uint64(1)) & O & m
    t |= (t << np.uint64(1)) & O & m; t |= (t << np.uint64(1)) & O & m; t |= (t << np.uint64(1)) & O & m; l |= (t << np.uint64(1)) & e
    t = (P >> np.uint64(1)) & O & m
    t |= (t >> np.uint64(1)) & O & m; t |= (t >> np.uint64(1)) & O & m; t |= (t >> np.uint64(1)) & O & m; l |= (t >> np.uint64(1)) & e
    t = (P << np.uint64(8)) & O
    t |= (t << np.uint64(8)) & O; t |= (t << np.uint64(8)) & O; t |= (t << np.uint64(8)) & O; l |= (t << np.uint64(8)) & e
    t = (P >> np.uint64(8)) & O
    t |= (t >> np.uint64(8)) & O; t |= (t >> np.uint64(8)) & O; t |= (t >> np.uint64(8)) & O; l |= (t >> np.uint64(8)) & e
    t = (P << np.uint64(7)) & O & m
    t |= (t << np.uint64(7)) & O & m; t |= (t << np.uint64(7)) & O & m; t |= (t << np.uint64(7)) & O & m; l |= (t << np.uint64(7)) & e
    t = (P >> np.uint64(7)) & O & m
    t |= (t >> np.uint64(7)) & O & m; t |= (t >> np.uint64(7)) & O & m; t |= (t >> np.uint64(7)) & O & m; l |= (t >> np.uint64(7)) & e
    t = (P << np.uint64(9)) & O & m
    t |= (t << np.uint64(9)) & O & m; t |= (t << np.uint64(9)) & O & m; t |= (t << np.uint64(9)) & O & m; l |= (t << np.uint64(9)) & e
    t = (P >> np.uint64(9)) & O & m
    t |= (t >> np.uint64(9)) & O & m; t |= (t >> np.uint64(9)) & O & m; t |= (t >> np.uint64(9)) & O & m; l |= (t >> np.uint64(9)) & e
    return l & _FULL_MASK
...
def get_flip(P, O, idx):
    m, mk, f = np.uint64(1) << np.uint64(idx), np.uint64(0x7E7E7E7E7E7E7E), np.uint64(0)
    ...
    if (t >> np.uint64(1)) & P: f |= t
    t = (m << np.uint64(8)) & O
    t |= (t << np.uint64(8)) & O; t |= (t << np.uint64(8)) & O; t |= (t << np.uint64(8)) & O
